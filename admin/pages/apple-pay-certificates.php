	...
	<form method="POST" class="apple_pay" action="<?php echo esc_url_raw( admin_url( 'admin-post.php' ) ); ?>" enctype="multipart/form-data">
		...
		<div class="form-row">
			...
			if ( isset( $apple_certificates['apple_certificate_path_file'] ) ) {
				echo '<label> Existing : ' . wp_kses_data( $apple_certificates['apple_certificate_path_file'] ) . ' <a href="' . esc_url(admin_url( 'options-general.php?page=apple-pay-certificates&delete_certificate=pathfile' )) . '" onclick="return confirm(\'Are you sure you want to delete this certificate?\')" class="delete_btn">Delete File</a></label>';
			}
			?>
		</div>
		...
		<div class="form-row">
			...
			if ( isset( $apple_certificates['apple_certificate_key_file'] ) ) {
				echo '<label> Existing : ' . wp_kses_data( $apple_certificates['apple_certificate_key_file'] ) . ' <a href="' . esc_url(admin_url( 'options-general.php?page=apple-pay-certificates&delete_certificate=keyfile' )) . '" onclick="return confirm(\'Are you sure you want to delete this certificate?\')" class="delete_btn">Delete File</a></label>';
			}
			?>
		</div>
