...
class WC_Gateway_APS_Super extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->redirection_text = __( 'You will be redirected to the Amazon Payment Services website when you place an order', 'amazon_payment_services' );

		$this->validation_messages = array(
			'required'   => __( '{field_name} : cannot be empty', 'amazon_payment_services' ),
			'no_space'   => __( '{field_name} : should not contain blank spaces', 'amazon_payment_services' ),
			'min_length' => __( '{field_name} : must be greater than or equals {min_length}', 'amazon_payment_services' ),
			'max_length' => __( '{field_name} : must be lower than or equals to {max_length}', 'amazon_payment_services' ),
		);
	...
	public function aps_handle_response( $response_mode = 'online', $integration_type = APS_Constants::APS_INTEGRATION_TYPE_REDIRECTION, $is_merchant_call = false ) {
		...
		if ( empty( $response_params ) ) {
			$response_params = json_decode( file_get_contents( 'php://input' ), true );
		}
	...
	public function aps_merchant_cancel() {
		...
		echo '<script>window.top.location.href = "' . esc_url( wc_get_checkout_url() ) . '"</script>';
		exit;
	}
	...
	public function aps_create_token( $response_params, $user_id ) {
		...
		try {
			if ( APS_Constants::APS_TOKEN_SUCCESS_RESPONSE_CODE === $response_params['response_code'] || APS_Constants::APS_TOKEN_SUCCESS_STATUS_CODE === $response_params['status'] ) {
				...
				$_SESSION['aps_token_error'] = $response_params['response_message'];
			}
		} catch ( Exception $e ) {
			$_SESSION['aps_token_error'] = $e->getMessage();
		}
	...
	public function show_checkout_payment_errors() {
		if ( isset( $_SESSION['aps_error'] ) ) {
			$aps_error = $_SESSION['aps_error'];
			/* translators: %s: aps_error */
			$aps_error_msg = sprintf( __( 'An error occurred while making the transaction. Please try again. (Error message: %s)', 'amazon_payment_services' ), $aps_error );
