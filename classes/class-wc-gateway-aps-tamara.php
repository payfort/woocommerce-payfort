 ...
 */
class WC_Gateway_APS_TAMARA extends WC_Gateway_APS_Super
{

    public function __construct()
    {
        parent::__construct();
        $this->id = APS_Constants::APS_PAYMENT_TYPE_TAMARA; // payment gateway plugin ID
        $this->has_fields = false; // in case you need a custom credit card form
        $this->method_title = __('Amazon Payment Service - Tamara', 'amazon-payment-services');
        $this->title = __('Pay in 4. No interest, no fees.', 'amazon-payment-services');
        $this->description = __('Amazon Payment Service - Tamara', 'amazon-payment-services');
        $this->method_description = __('Accept Tamara', 'amazon-payment-services'); // will be displayed on the options page
    ...
    {
        $available = 'yes' === $this->aps_config->get_enable_tamara() && in_array(strtoupper($this->aps_helper->get_front_currency()), $this->supported_currencies, true) ? 'yes' : 'no';
    ...
    {
        $tamara_token = filter_input(INPUT_POST, 'aps_payment_token_tamara');
        $order = new WC_Order($order_id);
        $tamara_mobile = filter_input(INPUT_POST, 'token_mobile_number');
        session_start();
        if (!empty($tamara_mobile)) {
            $_SESSION['tamara_payment']['mobile_number'] = wp_kses_data($tamara_mobile);
        }
        ...
        $payment_option = APS_Constants::APS_PAYMENT_METHOD_TAMARA;
    ...
    /**
     * Generate the TAMARA payment form
     *
     * @param none
     * @return string
     */
    public function payment_fields()
    {
        ...
        if (class_exists('APS_Public')) {
            APS_Public::load_tamara_wizard($this->aps_config->get_language(), $this->get_integration_type(), $this->get_enabled_tokenization(), $this->aps_config->have_subscription());
        }
    }
     ...
     */
     ...
     public function get_icon()
     {
        ...
        $tamara_logo = $image_directory . 'tamara-logo.png';
        //Wrap icons
        $icon_html .= '<img src="' . $tamara_logo . '" alt="tamara" class="payment-icons" />';
        $icon_html .= '</span>';
        return $icon_html;
    }
