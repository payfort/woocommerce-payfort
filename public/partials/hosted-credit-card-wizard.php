		...
		echo '<li>';
			echo '<div class="aps-row ' . wp_kses_data( $checked ) . '">';
				echo '<div class="aps-col-sm-9">';
					echo '<input type="radio" class="aps_cc_token aps_token_radio" name="aps_payment_token_cc" data-masking-card="' . esc_attr( substr( $masking_card, 0, 6 ) ) . '" data-token-id="' . esc_attr( $token_row->get_id() ) . '" value="' . esc_attr( $token_row->get_token() ) . '" ' . esc_attr($checked) . '/>';
