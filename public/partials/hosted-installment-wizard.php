		...
		echo '<li class="token_list">';
			echo '<div class="aps-row ' . wp_kses_data( $checked ) . '">';
				echo '<div class="aps-col-sm-9">';
					...
					echo wp_kses_data( __( 'exp', 'amazon_payment_services' ) . ' ' . $token_row->get_expiry_month() . '/' . $token_row->get_expiry_year() );
				echo '</div>';
	...
	<li>
		<div class="aps-row">
			<div class="aps-col-xs-12">
				<?php
				if ( ! empty( $tokens ) ) {
					echo '<input type="radio" name="aps_payment_token_installment" value="" class="aps_token_card aps_token_radio" required/> ' . __( 'Add a new card', 'amazon_payment_services' );
				}
				?>
