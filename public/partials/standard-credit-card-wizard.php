...
foreach ( $tokens as $token_row ) {
	...
	echo '<li>';
		echo '<div class="aps-row ' . wp_kses_data( $checked ) . '">';
			...
			echo '<input type="radio" class="aps_cc_token aps_token_radio" name="aps_payment_token_cc" data-masking-card="' . esc_attr( substr( $masking_card, 0, 6 ) ) . '" value="' . esc_attr( $token_row->get_token() ) . '" ' . esc_attr($checked) . '/>';
