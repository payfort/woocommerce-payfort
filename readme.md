...
|:--------------:|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|     2.4.6      | * Fix - Valu OTP verification now executes API call to APS for proper OTP validation <br/> * Fix - AJAX handler validates OTP verification response before allowing progression <br/> * Fix - Gateway currency is validated against enabled currencies instead of trusting the browser cookie <br/> * Fix - Payment amount is verified against the order total before an order is marked paid <br/> * Fix - Capture and Apple Pay refund now use the order currency for 0 and 3 decimal currencies <br/> * Fix - Order completion is locked to prevent duplicate processing from webhook and redirect <br/> * Fix - Tabby subscription renewals send payment_option instead of digital_wallet <br/> * Fix - Card security code, card number and expiry are redacted from debug logs <br/> * Fix - Apple Pay certificate and key are stored with owner only permissions <br/> * Fix - Apple Pay certificate directory is protected on Nginx as well as Apache <br/> * Fix - Token builder endpoint requires a logged in user and nonce verification <br/> * Fix - Nonce verification and rate limiting added to cart order creation <br/> * Fix - Nonce verification added to valU and Apple Pay AJAX endpoints <br/> * Fix - STC Pay mobile number format is validated before requesting an OTP <br/> * Fix - valU tenure selection validates the submitted tenure value <br/> * Fix - APS session data is cleared on logout <br/> * Fix - Checkout POST fields are sanitized before being stored <br/> * Fix - valU down payment configuration value is escaped on output.                                                                                                                                                                                                   |
...
|     2.4.1      | * Fix - ApplePay signature and Url validation fix.                                                                                                                                                                                                                                                                                                                                 |
